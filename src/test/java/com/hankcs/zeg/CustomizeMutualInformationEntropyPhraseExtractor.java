 ...
 * @author hankcs
 */
public class CustomizeMutualInformationEntropyPhraseExtractor implements IPhraseExtractor
{
    private AbstractLexicalAnalyzer lexicalAnalyzer;

    public CustomizeMutualInformationEntropyPhraseExtractor() throws IOException {
        lexicalAnalyzer = new PerceptronLexicalAnalyzer();
    }

    public CustomizeMutualInformationEntropyPhraseExtractor(AbstractLexicalAnalyzer lexicalAnalyzer) {
    ...
    public List<String> extractPhrase(String text, int size)
    {
        ...
        for (List<Term> sentence : filterSeg2Sentence(text, filterChain))
        {
    ...
    private List<List<Term>> filterSeg2Sentence(String text, Filter[] filterChain) {
        List<List<Term>> seg2sentence = lexicalAnalyzer.seg2sentence(text);
        for (List<Term> sentence : seg2sentence)
        {
     ...
     * @return
     */
    public static List<String> extract(String text, int size) throws IOException {
        IPhraseExtractor extractor = new CustomizeMutualInformationEntropyPhraseExtractor();
