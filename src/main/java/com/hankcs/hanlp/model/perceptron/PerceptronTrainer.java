...
public abstract class PerceptronTrainer extends InstanceConsumer
{
    ...
    /**
     *训练并将训练后的模型保存到OSS
     * @param contents 待训练语料
     * @param bucket OSS的bucket
     * @param modelFile OSS的key
     * @param compressRatio
     * @param maxIteration
     * @param threadNum
     * @return
     ...
     * @date 2022/9/6 14:47
     */
    public Result train2Oss(List<String> contents,
            String bucket, String modelFile, final double compressRatio,
            final int maxIteration, final int threadNum) throws IOException
    {
        ...
        ConsoleLogger logger = new ConsoleLogger();
        logger.start("开始加载训练集...\n");
        Instance[] instances = loadTrainInstancesByContents(contents, mutableFeatureMap);
        tagSet.lock();
        logger.finish("\n加载完毕，实例一共%d句，特征总数%d\n", instances.length, mutableFeatureMap.size() * tagSet.size());
        ...
        if (threadNum == 1 || !Config.TrainingMultiThreading)
        {
            AveragedPerceptron model;
            model = new AveragedPerceptron(immutableFeatureMap);
            final double[] total = new double[model.parameter.length];
            ...
            for (int iter = 1; iter <= maxIteration; iter++)
            {
                ...
                // 在开发集上校验
                accuracy = IOUtility.evaluate(instances, model);
                out.printf("Iter#%d - ", iter);
                printAccuracy(accuracy);
            }
            // 平均
            ...
            printAccuracy(accuracy);
            logger.start("以压缩比 %.2f 保存模型到 %s ... ", compressRatio, modelFile);
            model.save2Oss(bucket, modelFile, immutableFeatureMap.featureIdMap.entrySet(), compressRatio);
            logger.finish(" 保存完毕\n");
            if (compressRatio == 0) return new Result(model, accuracy);
        }
        else
        {
            ...
            for (int iter = 1; iter <= maxIteration; iter++)
            {
                ...
                try
                {
                    ...
                    accuracy = IOUtility.evaluate(instances, models[0]);
                    out.printf("Iter#%d - ", iter);
                    printAccuracy(accuracy);
                }
                catch (InterruptedException e)
            ...
            logger.start("以压缩比 %.2f 保存模型到 %s ... ", compressRatio, modelFile);
            models[0].save2Oss(bucket, modelFile, immutableFeatureMap.featureIdMap.entrySet(), compressRatio,
                    HanLP.Config.DEBUG);
            logger.finish(" 保存完毕\n");
            if (compressRatio == 0) return new Result(models[0], accuracy);
        }
        ...
        if (compressRatio > 0)
        {
            accuracy = evaluateByContents(contents, model);
            out.printf("\n%.2f compressed model - ", compressRatio);
            printAccuracy(accuracy);
        }
    ...
    protected Instance[] loadTrainInstancesByContents(List<String> contents,
            final MutableFeatureMap mutableFeatureMap) throws IOException
    {
        final List<Instance> instanceList = new LinkedList<Instance>();
        IOUtility.loadInstanceByContents(contents, new InstanceHandler()
        {
            @Override
...
//            out.printf("Finished [%d,%d)\n", start, end);
    ...
    public Result train2Oss(List<String> contents, String bucket, String modelFile) throws IOException
    {
        return train2Oss(contents, bucket, modelFile, 0.1, 50, Runtime.getRuntime().availableProcessors());
    }
